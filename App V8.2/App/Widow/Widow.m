...
%                             0,sinalpha,cosalpha];
        ...
        function drawWorkspace(obj,thlim)
            hold on;
            cla;
            th1 = linspace(-double(thlim(1)),double(thlim(1)), 80.0)*pi/180;
            th2 = linspace(-double(thlim(2)),double(thlim(2)), 20.0)*pi/180;
            th3 = linspace(-double(thlim(3)),double(thlim(3)), 20.0)*pi/180;
            th4 = linspace(-double(thlim(4)),double(thlim(4)), 40.0)*pi/180;            
            [t1,t2,t3,t4] = ndgrid(th1,th2,th3,th4);
            
            %Terna traslación de cinematica inversa EE respecto a origen 0.
            xM = cos(t1).*(obj.L4*cos(t2 + t3)+ obj.Lp*cos(t2) + obj.Lee*cos(t2+t3+t4));
            yM = sin(t1).*(obj.L4*cos(t2 + t3)+ obj.Lp*cos(t2) + obj.Lee*cos(t2+t3+t4));
            zM = obj.L1 + obj.L4*sin(t2 + t3) + obj.Lp*sin(t2) + obj.Lee*sin(t2 + t3 + t4);
            plot3(xM(:),yM(:),zM(:),'o')
            hold off;
        end
